...
// --- Fan Settings (IRF520 Module on GPIO 18) ---
const int fanPin = 18;
const int fanChannel = 1;         // Different PWM channel from buzzer
const int fanFreq = 5000;         // 5kHz PWM frequency for fan
const int fanResolution = 8;      // 8-bit resolution (0-255)
...
void loop() {
  ...
  if (currentTemp > TEMP_THRESHOLD || manualAlarm) {
    // Overheat or Manual Alarm triggered -> Force MAX Speed
    fanSpeed = 255;
  } else {
    // Normal operation -> Keep fan fully OFF
    fanSpeed = 0;
  }
...
// Select PWM function based on Core version
      ...
      // Fan is now handled continuously at the start of loop
